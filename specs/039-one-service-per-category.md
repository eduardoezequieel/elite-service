...
5. El seed deja un segundo rubro con servicios (`Pulido de silvines`, precios de ejemplo) para que el agrupado tenga más de un grupo que sumar.

## Done

- [x] `TicketForm` pinta un `radiogroup` por categoría activa con servicios; no aplana el catálogo.
- [x] Elegir un servicio de un rubro no suelta el de otro; elegir otro del mismo rubro reemplaza al anterior.
- [x] Tocar el ya elegido lo deselecciona. Sin ningún servicio, no se abre el ticket.
- [x] `EditTicketDialog` usa la misma regla.
- [x] Una categoría sin servicios activos no se pinta. Una de un solo servicio se toca o se suelta igual.
- [x] El resumen lista cada línea y el total es la suma. El descuento es por línea.
- [x] `POST/PATCH` de tickets (pista y oficina) responde 422 `DUPLICATE_SERVICE_CATEGORY` si vienen dos `serviceId` de la misma categoría.
- [x] Tests de la agrupación (`service-groups.spec.ts`) y de la validación (`build-ticket-items.spec.ts`). No hay nombres de categoría en el código.
