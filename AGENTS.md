...
| **Render** (`render.yaml`, plan free) | API. Health `/api/health`. Al arrancar: `prisma migrate deploy` + seed. `PORT` lo inyecta Render.                                                                                                                                                                                                                                     | `DATABASE_URL` (Neon **directa**, `sslmode=require`, sin `-pooler`), `JWT_SECRET`, `PIN_PEPPER` (spec 044: sin esto el API no arranca, y cambiarlo invalida todos los PINs), `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `WEB_ORIGIN` (URL de Vercel), `NODE_ENV=production`. |
